...
function TourismDetailPage() {
  ...
  const editorialClassified = useMemo(() => {
    ...
    const gallery =
      Array.isArray(experience.gallery_urls) && experience.gallery_urls.length > 0
        ...
        {issuedVoucher ? (
          <div className="py-6 text-center space-y-4">
            ...
            <div className="space-y-1">
              <h4 className="text-base font-bold text-foreground">Voucher Confirmado</h4>
          ...
          >
            ...
            {totalPriceCents > 0 && (
              <div className="p-3.5 rounded-2xl bg-muted/50 flex items-center justify-between text-xs font-bold">
                ...
                <span className="text-sm font-black text-foreground">{formatMoney(totalPriceCents)}</span>
              </div>
            )}
  ...
  // ── Rule 23: Modo Proprietário Canônico derivado da sessão segura ────
  const isOwner = Boolean(
    (session?.id || session?.user?.id) &&
      (session.id === (experience as any)?.author_profile_id ||
        session.user?.id === (experience as any)?.author_profile_id ||
        session.id === (experience as any)?.user_id ||
        session.user?.id === (experience as any)?.user_id ||
        session.role === "platform_admin" ||
        session.store_id === experience?.store_id ||
  ...
  return (
    <>
      <div className="w-full max-w-6xl mx-auto space-y-6 pb-24 px-0 sm:px-4 md:px-0">
        <ProductTelemetry
        ...
        <div className="space-y-3">
          <div className="flex flex-wrap items-center gap-2">
            <Badge variant="outline" className="text-xs font-medium bg-muted/60 text-foreground">
          ...
          {experience.subtitle && (
            <p className="text-sm sm:text-base text-muted-foreground font-medium">{experience.subtitle}</p>
          )}
        ...
        <TravelQuoteModal
          ...
          defaultTripType={
            experience.category === "hospedagens"
              ? "hotel_only"
              : (experience.category as string) === "cruzeiro"
                ? "cruise"
                : "air_package"
          }
        />
      ...
      {/* ── Sticky Bottom CTA Bar (Mobile Only) — Regra 12: Thumb Zone / 3 Toques ── */}
