...
const sqlPath = path.resolve(__dirname, "../supabase/migrations/20261106000000_central_knowledge_professions_services_holidays_demographics.sql");
...
async function run() {
  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      console.log("Executando migração 20261106000000 no Supabase...");
      await sql.unsafe(sqlContent);
      console.log("Migração 20261106000000 aplicada com sucesso no Supabase!");
      await sql.end();
      success = true;
      break;
    } catch (err) {
  ...
  if (!success) {
    console.error("Não foi possível aplicar a migração no Supabase.");
    process.exit(1);
  }
}
