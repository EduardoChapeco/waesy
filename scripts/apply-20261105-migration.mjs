...
const sqlPath = path.resolve(__dirname, "../supabase/migrations/20261105000000_central_knowledge_logistics_units_payments_realestate.sql");
...
async function run() {
  ...
  for (const cfg of configs) {
    ...
    try {
      ...
      console.log("Query OK! Aplicando migration 20261105000000 (Logistics, Units of Measure, Payments, Real Estate, Holidays)...");
      await sql.unsafe(sqlContent);
      console.log("Migration 20261105000000 aplicada com sucesso!");
  ...
  if (!success) {
    console.error("Não foi possível aplicar a migration em nenhuma porta.");
    process.exit(1);
  }
}
